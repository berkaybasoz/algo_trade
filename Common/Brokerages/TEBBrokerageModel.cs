...
{
    /// <summary>
    /// Provides TEB specific properties
    /// </summary>
    public class TEBBrokerageModel : DefaultBrokerageModel
    {
        ...
        public TEBBrokerageModel(AccountType accountType = AccountType.Margin)
            : base(accountType)
        {
        }
        ...
        public override bool CanSubmitOrder(Security security, Order order, out BrokerageMessageEvent message)
        {
            ...
            if (!CanExecuteOrder(security, order))
            {
                message = new BrokerageMessageEvent(BrokerageMessageType.Warning, "ExtendedMarket",
                    "TEB does not support extended market hours trading.  Your order will be processed at market open."
                    );
            }
        ...
        public override bool CanUpdateOrder(Security security, Order order, UpdateOrderRequest request, out BrokerageMessageEvent message)
        {
            ...
            // TEB doesn't allow updating order quantities
            if (request.Quantity != null && request.Quantity != order.Quantity)
            {
                message = new BrokerageMessageEvent(BrokerageMessageType.Warning, "UpdateRejected",
                    "TEB does not support updating order quantities."
                    );
        ...
        public override bool CanExecuteOrder(Security security, Order order)
        {
            ...
            // TEB doesn't support after hours trading
        ...
        {
            // TEB cancels reverse splits
            var splitFactor = split.SplitFactor;
            if (splitFactor > 1.0m)
            {
                tickets.ForEach(ticket => ticket.Cancel("TEB Brokerage cancels open orders on reverse split symbols"));
            }
            else
            {
        ...
        {
            return new TEBSlippageModel(0.0001M);
        }
