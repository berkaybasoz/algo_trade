...
namespace QuantConnect.ToolBox.GoogleDownloader
{
    ...
    public class GoogleDataDownloader : IDataDownloader
    {
        ...
        /// <param name="endUtc">End time of the data in UTC</param>
        /// <returns>Enumerable of base data for this symbol</returns>
        public IEnumerable<BaseData> Parse(String path, Symbol symbol, Resolution resolution, DateTime startUtc, DateTime endUtc)
        {// First 7 lines are headers 
            var currentLine = 9;
            ...
            { 
                    ...
                    if (str.Length >= 6)
                    {
                        ...
                        var startTime = FromUnixTime(str[0].Remove(0, 1).ToInt64());
                        ...
                        datas.Add(new TradeBar(time, symbol, open, high, low, close, volume, resolution.ToTimeSpan()));
                    }
                    currentLine++; 
            }
            return datas;
